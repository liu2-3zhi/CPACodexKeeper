...
class MaintainerTests(unittest.TestCase):
    ...
    def test_run_fill_forever_logs_next_poll_interval(self):
        ...
        self.assertIn(("INFO", "fill 模式启动，执行间隔: 10 秒", 0), messages)
        self.assertIn(("INFO", "开始第 1 轮 fill 巡检", 0), messages)
        self.assertIn(("INFO", "第 1 轮 fill 巡检结束", 0), messages)
        self.assertIn(("INFO", "等待 10 秒后开始下一轮 fill 巡检", 0), messages)
