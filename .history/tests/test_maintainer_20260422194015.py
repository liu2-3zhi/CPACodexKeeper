...
class MaintainerTests(unittest.TestCase):
    ...
    def test_init_uses_state_dir_for_disabled_and_delete_blocked_paths(self):
        state_dir = pathlib.Path(self.temp_dir.name) / "state"
        maintainer = CPACodexKeeper(
            settings=Settings(
                cpa_endpoint="https://example.com",
                cpa_token="secret",
                state_dir=state_dir,
        ...
        self.assertEqual(maintainer.disabled_accounts_path, state_dir / "disabled_accounts.json")
        self.assertEqual(maintainer.delete_blocked_accounts_path, state_dir / "delete_blocked_accounts.json")
    ...
    def test_process_token_force_refreshes_enabled_token_by_temporarily_disabling_and_reenabling(self):
        self.maintainer.settings.force_refresh_on_expiry = True
        near_expiry = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.maintainer.get_token_detail = Mock(return_value={
            "email": "a@example.com",
            "disabled": False,
        ...
        result = self.maintainer.process_token({"name": "t-force-enabled"}, 1, 1)

        self.assertEqual(result, "alive")
        self.assertEqual(
            self.maintainer.set_disabled_status.call_args_list,
            [
                call("t-force-enabled", disabled=True, logger=ANY),
                call("t-force-enabled", disabled=False, logger=ANY),
            ],
        )
        self.maintainer.upload_updated_token.assert_called_once()
        self.assertEqual(self.maintainer.stats.refreshed, 1)

    def test_process_token_force_refreshes_disabled_token_and_keeps_disabled(self):
        self.maintainer.settings.force_refresh_on_expiry = True
        near_expiry = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.maintainer.get_token_detail = Mock(return_value={
            "email": "a@example.com",
        ...
        result = self.maintainer.process_token({"name": "t-force-disabled"}, 1, 1)
    ...
    def test_process_token_force_refresh_skips_when_refresh_token_is_missing(self):
        self.maintainer.settings.force_refresh_on_expiry = True
        near_expiry = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.maintainer.get_token_detail = Mock(return_value={
            "email": "a@example.com",
        ...
        self.maintainer.upload_updated_token = Mock()
        self.maintainer.set_disabled_status = Mock(return_value=True)

        result = self.maintainer.process_token({"name": "t-force-no-rt"}, 1, 1)

        self.assertEqual(result, "alive")
        self.maintainer.try_refresh.assert_not_called()
        self.maintainer.upload_updated_token.assert_not_called()
        self.maintainer.set_disabled_status.assert_not_called()
        self.assertEqual(self.maintainer.stats.refreshed, 0)

    def test_process_token_force_refresh_skips_when_remaining_lifetime_is_above_threshold(self):
        self.maintainer.settings.force_refresh_on_expiry = True
        far_expiry = (datetime.now(timezone.utc) + timedelta(days=10)).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.maintainer.get_token_detail = Mock(return_value={
            "email": "a@example.com",
            "disabled": False,
            "access_token": "token",
        ...
        result = self.maintainer.process_token({"name": "t-force-far"}, 1, 1)
    ...
    def test_process_token_force_refresh_failure_after_temporary_disable_leaves_token_disabled(self):
        self.maintainer.settings.force_refresh_on_expiry = True
        near_expiry = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.maintainer.get_token_detail = Mock(return_value={
            "email": "a@example.com",
        ...
        self.maintainer.try_refresh = Mock(return_value=(False, None, "刷新失败"))
        self.maintainer.upload_updated_token = Mock(return_value=True)
        self.maintainer.set_disabled_status = Mock(return_value=True)

        result = self.maintainer.process_token({"name": "t-force-failed"}, 1, 1)

        self.assertEqual(result, "alive")
        self.assertEqual(
            self.maintainer.set_disabled_status.call_args_list,
            [call("t-force-failed", disabled=True, logger=ANY)],
        )
        self.maintainer.upload_updated_token.assert_not_called()
        self.assertEqual(self.maintainer.stats.refreshed, 0)

    def test_process_token_override_false_skips_env_activated_force_refresh(self):
        self.maintainer.settings.force_refresh_on_expiry = True
        near_expiry = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.maintainer.get_token_detail = Mock(return_value={
            "email": "a@example.com",
        ...
        result = self.maintainer.process_token({"name": "t-force-override-off"}, 1, 1, force_refresh_on_expiry=False)
    ...
    def test_log_startup_includes_usage_query_interval(self):
        ...
        self.assertIn(f"日志巡检轮询间隔: {self.settings.fill_interval_seconds} 秒", joined)
        self.assertIn(f"日志巡检查询窗口: {self.settings.usage_query_interval_seconds} 秒", joined)
    ...
    def test_log_startup_marks_usage_query_interval_disabled_when_zero(self):
        ...
        joined = "\n".join(captured)
        self.assertIn(f"日志巡检轮询间隔: {self.settings.fill_interval_seconds} 秒", joined)
        self.assertIn("日志巡检查询窗口: 已禁用（CPA_USAGE_QUERY_INTERVAL=0）", joined)
    ...
    def test_fill_mode_keeps_query_cursor_when_usage_log_fetch_fails(self):
    ...
    def test_fill_mode_catches_late_arriving_usage_entries_on_next_poll(self):
        self.maintainer.last_usage_query_time = 1000
        ...
        self.maintainer.get_usage_log = Mock(side_effect=[
            {
                "usage": {
                    "apis": {
                        "api-1": {
                            "models": {
                                "gpt-5.3-codex": {
                                    "details": [
                                        {"source": "a@example.com", "timestamp": "1970-01-01T00:18:15+00:00"},
                                    ]
                                }
                            }
                        }
                        ...
                        "api-1": {
                            "models": {
                                "gpt-5.3-codex": {
                                    "details": [
                                        {"source": "a@example.com", "timestamp": "1970-01-01T00:18:15+00:00"},
                                        {"source": "b@example.com", "timestamp": "1970-01-01T00:18:18+00:00"},
                                    ]
                                }
                            }
                        }
        ...
        self.maintainer.process_fill_token = Mock(side_effect=["alive", "alive"])

        with patch("src.maintainer.time.time", return_value=1100):
            first_result = self.maintainer.run_fill_once()
        with patch("src.maintainer.time.time", return_value=1110):
            second_result = self.maintainer.run_fill_once()

        self.assertEqual(first_result, "processed")
        self.assertEqual(second_result, "processed")
        self.assertEqual(self.maintainer.last_usage_query_time, 1110)
        self.assertEqual(
            self.maintainer.process_fill_token.call_args_list,
            [
                call({"name": "token-a", "type": "codex", "email": "a@example.com"}, 1, 1),
                call({"name": "token-b", "type": "codex", "email": "b@example.com"}, 1, 1),
            ],
        )
    ...
    def test_run_emits_detailed_summary(self, _time_mock, executor_cls, as_completed_mock, _shuffle_mock):
        ...
        self.assertTrue(any("账号总数: 2" in line and "][INFO]:" in line for line in summary))
