...
- 当 `CPA_USAGE_QUERY_INTERVAL > 0` 时，会额外启动基于 `/v0/management/usage` 的日志巡检；它会对比上次查询时间，只挑出这段时间内新增调用过、且仍存在于过滤后 auth-files 中的 codex 账号，再对这些账号执行额度检查。日志巡检只负责禁用，不负责启用、刷新或删除。
