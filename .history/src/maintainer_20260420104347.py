...
class CPACodexKeeper:
    PAUSE_MESSAGES = {
        ("full", "log"): "优先级协调：日志巡检正在等待，主巡检将在当前账号完成后暂停",
        ("full", "timer"): "优先级协调：定时复查正在等待，主巡检将在当前账号完成后暂停",
        ("log", "timer"): "优先级协调：定时复查正在等待，日志巡检将在当前账号完成后暂停",
    }
    ...
    def run(self):
        ...
        if not tokens:
            self.log("WARN", "主巡检未发现任何可处理的 codex Token")
            return
        ...
        total = len(tokens)
        self.log("INFO", f"主巡检任务已就绪：本轮共 {total} 个 codex Token 待处理")
    ...
    def run_forever(self, interval_seconds=1800):
        ...
        while True:
            round_no += 1
            self.log("INFO", f"主巡检第 {round_no} 轮开始：准备扫描全部 codex Token")
            try:
                self.run()
                self.log("INFO", f"主巡检第 {round_no} 轮结束：已完成本轮全部 Token 扫描")
    ...
    def run_fill_once(self):
        if self.settings.usage_query_interval_seconds == 0:
            self.log("INFO", "日志巡检已禁用：CPA_USAGE_QUERY_INTERVAL=0，跳过本轮 usage 扫描")
            return "disabled"

        now = int(time.time())
        if self.last_usage_query_time is None:
            self.last_usage_query_time = now
            self.log("INFO", "日志巡检首次启动：已记录起始查询时间，下一轮开始比对新增 usage")
            return "primed"
        ...
        if not usage_data:
            ...
            self.log("WARN", "日志巡检未获取到 usage 数据：本轮无法筛选新增调用账号")
            return "skipped"
    ...
    def run_fill_forever(self, interval_seconds=10):
        ...
        while True:
            round_no += 1
            self.log("INFO", f"日志巡检第 {round_no} 轮开始：准备扫描新增 usage 日志")
            try:
                self.run_fill_once()
                self.log("INFO", f"日志巡检第 {round_no} 轮结束：已完成本轮 usage 日志扫描")
            except KeyboardInterrupt:
                raise
            except Exception as exc:
                self.log("ERROR", f"日志巡检第 {round_no} 轮异常: {exc}")
            self.log("INFO", f"日志巡检休眠中：{interval_seconds} 秒后开始下一轮 usage 日志扫描")
            time.sleep(interval_seconds)
